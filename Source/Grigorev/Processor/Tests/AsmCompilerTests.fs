...
type public YaccTests () =
    ...
    member this.Test0 () =
        ...
        Assert.That ((fun () -> match comp.Compile code with | Error _ -> true | _ -> false), Is.EqualTo(true))
    ...
    member this.Test1 () =
        ...
        let code = [| thr |]
        let comp = new AsmYaccCompiler<int> ()
        Assert.That ((fun () -> match comp.Compile code with | Error _ -> true | _ -> false), Is.EqualTo(false))